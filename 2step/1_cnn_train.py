...
class CropBuildingDatasetWithShapeBinary(Dataset):
    def __init__(self, csv_path, transform=None, image_size=224):
        ...
        self.labels = self.df["binary_label"].tolist()
...
def plot_training_curves(train_losses, val_accuracies, save_dir, disaster_type):
    ...
    plt.legend()
    plt.savefig(os.path.join(save_dir, f"loss_curve_{disaster_type}.png"))
    plt.close()
    ...
    plt.legend()
    plt.savefig(os.path.join(save_dir, f"accuracy_curve_{disaster_type}.png"))
    plt.close()

# === train ===
def train_one_type(train_csv, val_csv, disaster_type, save_dir, device, epochs=30, patience=5):
    train_dataset = CropBuildingDatasetWithShapeBinary(train_csv)
    val_dataset = CropBuildingDatasetWithShapeBinary(val_csv)
    ...
    for epoch in range(epochs):
        ...
        if val_acc > best_val_acc:
            ...
            model_path = os.path.join(save_dir, f"best_model_type_{disaster_type}.pt")
...
if __name__ == "__main__":
    ...
    train_val_dir = "../type_train_val_split_with_shape_binary"
    save_dir = "../binary_cnn_models"
    ...
    for disaster_type in type_list:
        ...
        train_one_type(train_csv, val_csv, disaster_type, save_dir, device, epochs=30, patience=3)
